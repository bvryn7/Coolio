...
const AccountMainContent: React.FC = () => {
  const { user } = useUser();
  // Ensure user is not null before using its properties
  const [name, setName] = useState<string | undefined>(user?.name);
  const navigate = useNavigate();

  useEffect(() => {
    if (user) {
      setName(user.name); // Update name if user is available
    }
  }, [user]);
    ...
    <div className="container mx-auto p-4">
      {user ? (
        ...
        <div className="flex justify-center items-center h-full">
          <p className="text-lg text-gray-600">Please log in to manage your account</p>
        </div>
      )}
    </div>
  );
